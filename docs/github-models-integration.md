...
3. Модель по умолчанию — `microsoft/phi-4-mini-instruct`; запрос требует строгий JSON с кратким русским резюме и `low`/`medium`/`high`. Результат валидируется Pydantic, а размер текста ограничен.
