...
3. Модель по умолчанию — `openai/gpt-4o` через GitHub Models (не OpenAI API); она задаётся переменной `AI_MODEL`. Запрос требует JSON Schema с кратким русским резюме и `low`/`medium`/`high`, а результаты пакета сопоставляются по стабильному идентификатору. Результат валидируется Pydantic, а размер текста ограничен.
...
6. Временные ошибки сети, `429` и `5xx` повторяются ограниченное число раз с учётом `Retry-After`; после исчерпания попыток AI отключается до конца запуска. Ошибка авторизации или невалидный ответ также отключают AI. Публикация отправляется через старый детерминированный путь. Платный fallback отсутствует.
